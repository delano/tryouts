  ...
  class CLI
    ...
    class QuietFormatter
      ...
      def file_end(_file_path, context_info: {})
        @stderr.puts # add newline after all dots
      end
      ...
      def test_result(result_packet)
        ...
        when :passed
          @stderr.print Console.color(:green, '.')
        when :failed
          @stderr.print Console.color(:red, 'F')
        when :error
          @stderr.print Console.color(:red, 'E')
        when :skipped
          @stderr.print Console.color(:yellow, 'S')
        else
          @stderr.print '?'
        end
        @stderr.flush
      ...
      def grand_total(total_tests:, failed_count:, error_count:, successful_files:, total_files:, elapsed_time:)
        ...
        @stderr.puts
        @stderr.puts # Add newline after dots
        ...
        if issues_count > 0
          ...
          @stderr.puts Console.color(:red, "Total: #{details.join(', ')}, #{passed} passed (#{time_str})")
        else
          @stderr.puts Console.color(:green, "Total: #{total_tests} passed (#{time_str})")
        end
